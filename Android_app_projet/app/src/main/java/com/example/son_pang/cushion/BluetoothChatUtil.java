...
public class BluetoothChatUtil {
    ...
    public void registerHandler(android.os.Handler handler){
        mHandler = handler;
    }
     ...
     */
    private synchronized void setState(int state) {        //相当于内部锁把state状态锁住
        Log.d(TAG, "setState() " + mState + " -> " + state);
        mState = state;
        // 给新状态的处理程序，界面活性可以更新
        mHandler.obtainMessage(STATE_CHANGE, state, -1).sendToTarget();//类似于handler.sendMessage  //arg:-1的意思未知
    }
        ...
        public AcceptThread() {
            ...
            try {
                tmp = mAdapter.listenUsingRfcommWithServiceRecord(
                        SERVICE_NAME, SERVICE_UUID);  //升阶书P392
    ...
    private class ConnectedThread extends Thread {
        ...
        public void run() {
            ...
            while (true) {
                try {/*
            //byte[] buffer = new byte[3];//原本是byte[] buffer = new byte[1024];   数据缓冲
            // 读取输入流
           //int bytes = mmInStream.read(buffer);
            ...
            Bundle bundle = new Bundle();
            bundle.putByteArray(READ_MSG, buffer);//bundle.putByteArray(READ_MSG, buffer);
            msg.setData(bundle);
            mHandler.sendMessage(msg);*/
            /*if ((bytes = mmInStream.read(buffer))>0){
                ...
                Message msg = mHandler.obtainMessage(MESSAGE_READ);
                Bundle bundle = new Bundle();
                bundle.putByteArray(READ_MSG,buf_data);
                msg.setData(bundle);
                mHandler.sendMessage(msg);
            }*/
                    if ((bytes = mmInStream.read(buffer))>0){
